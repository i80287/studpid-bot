...
async def write_log_async(filename: str, *report: str) -> None:
    ...
    async with aiofiles.open(CWD_PATH + '/' + filename, mode="a+", encoding="utf-8") as f:
        await f.write(f"[{datetime.utcnow() + _TZ_OFFSET}] {' '.join(('[' + s + ']' for s in report if s))}\n")
...
async def write_guild_log_async(filename: str, guild_id: int, report: str) -> None:
    ...
    async with aiofiles.open(CWD_PATH + f"/logs/logs_{guild_id}/" + filename, mode="a+", encoding="utf-8") as f:
        await f.write(f"[{datetime.utcnow() + _TZ_OFFSET}] {report}\n")
